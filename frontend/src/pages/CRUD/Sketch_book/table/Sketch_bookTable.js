...
import * as dataFormat from 'pages/CRUD/Sketch_book/table/Sketch_bookDataFormatters';

import actions from 'actions/sketch_book/sketch_bookListActions';
...
const Sketch_bookTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    { label: 'Sketches', title: 'sketches' },
    { label: 'Created Date', title: 'createdDate' },
  ]);
  ...
  const count = useSelector((store) => store.sketch_book.list.count);
  const modalOpen = useSelector((store) => store.sketch_book.list.modalOpen);
  const rows = useSelector((store) => store.sketch_book.list.rows);
  const idToDelete = useSelector((store) => store.sketch_book.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>Sketch_book</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/sketch_book/new'>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/sketch_book/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Sketch_book'
                : 'http://localhost:8080/api-docs/#/Sketch_book'
            }
          >
            API documentation for sketch_book
          </LinkMaterial>
        </div>
      </Widget>
...
export default Sketch_bookTable;
