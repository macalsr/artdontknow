...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/sketch_book/autocomplete`, { params });
  return response.data;
}

const Sketch_bookSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.sketch_book.hasPermissionToCreate,
});

export default connect(select)(Sketch_bookSelectItem);
